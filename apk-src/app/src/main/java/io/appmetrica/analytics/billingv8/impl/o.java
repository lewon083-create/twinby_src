...
public final class o implements UpdatePolicy {
    ...
    public final SystemTimeProvider f21696a;
    ...
    public final Map<String, BillingInfo> getBillingInfoToUpdate(@NotNull BillingConfig billingConfig, @NotNull Map<String, ? extends BillingInfo> map, @NotNull BillingInfoManager billingInfoManager) {
        ...
        for (Map.Entry<String, ? extends BillingInfo> entry : map.entrySet()) {
            ...
            if (value.type != ProductType.INAPP || billingInfoManager.isFirstInappCheckOccurred()) {
                ...
                if (((billingInfo == null || !Intrinsics.a(billingInfo.purchaseToken, value.purchaseToken)) && (this.f21696a.currentTimeMillis() - value.purchaseTime <= TimeUnit.SECONDS.toMillis(billingConfig.firstCollectingInappMaxAgeSeconds) || value.type == ProductType.SUBS)) || (billingInfo != null && value.type == ProductType.SUBS && this.f21696a.currentTimeMillis() - billingInfo.sendTime >= TimeUnit.SECONDS.toMillis(billingConfig.sendFrequencySeconds))) {
                    linkedHashMap.put(entry.getKey(), entry.getValue());
                }
            } else if (this.f21696a.currentTimeMillis() - value.purchaseTime <= TimeUnit.SECONDS.toMillis(billingConfig.firstCollectingInappMaxAgeSeconds)) {
                linkedHashMap.put(entry.getKey(), entry.getValue());
            }
        }
    ...
    public o(@NotNull SystemTimeProvider systemTimeProvider) {
        this.f21696a = systemTimeProvider;
    }
